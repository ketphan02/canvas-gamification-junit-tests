...
public class RandomUtil {
    /*
     * Note: not guaranteed to fail cases where delta(user range, expected range) is
     * small
     */
    public static final int NO_BIN = -1;
    public static final double ALPHA_LEVEL = 0.12;
    /**
     * This constant is the ratio of bin frequencies that must be within the range
     * for the group of bin frequencies to be
     * considered to represent a random set
     */
    public static final double ACCEPTANCE_RATE = 0.90;

    /**
     * Returns if the set of values described by the total number of values
     * considered and an array of "bin sizes" is
     * uniform. "Bin counts" refers to the number of values that are placed in each
     * bin. This method assumes that the
     * size (range) of each bin is equal.
     *
     * @param numValues:   Total number of values in the set.
     * @param binSizes:    The number of values that are placed in each
     *                     equally-sized bin.
    ...
    public static boolean describesUniform(int numValues, long[] binSizes, double alpha_level) {
        ...
        // function returns false if we fail to reject null hypothesis, which implies
    ...
    public static int assignedBinIndex(int value, int lower, int upper, int numBins) {
        ...
        assertWithinRange(value, lower, upper,
                "One or more of your randomly generated numbers fall outside of the required range.");
    ...
    public static RandomClause<?> castRandomClause(Clause clause) {
        if (!(clause instanceof RandomClause))
            return null;
    ...
    public static boolean frequenciesAreRandom(int[] frequencies, int numBins) {
        // assume that the sum of frequencies in this list == the total number of values
        // generated
        ...
        Logger.logMessage("DEBUG: " + ((count * 1.0) / numBins) + "");
        // If the number of bins which pass the randomness check is above the acceptance
        // rate, the numbers are considered random
