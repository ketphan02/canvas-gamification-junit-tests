...
public abstract class BaseTest {
    ...
    public void setRegexSentence(Clause[] regexSentence) throws InvalidClauseException {
        // By using a set we can ensure that there are no duplicates in the
        // regexSentence
        ...
        for (int i = 0; i < regexSentence.length; i++) {
            ...
            try {
                ...
                throw new InvalidClauseException(
                        "The parameter(s) for the clause at index " + (i + 1) + " are invalid. " + e.getMessage());
            }
        }
    ...
    public void setInjectedClauses(Clause[] injectedClauses) {
        if (injectedClauses != null) {
            if (injectedClauses.length != placeHolderCount(testSentence())) {
                // TODO: check if failing here stops errors from invalid TestOptions from being
                // a problem
                _fail("There is an issue with the test definition. Please contact an administrator.",
    ...
    public String getItemAtIndex(int index) {
        ...
        try {
            if (matcher.find())
                ...
                fail("Your code's output did not follow the correct structure/syntax");
    ...
    public void outputFollowsCorrectStructure() {
        ...
        _assertTrue(matcher.find(), incorrectOutputMessage,
                parseTestInformation(output, getRegexSentence(), incorrectOutputMessage));
        ...
        for (Clause clause : getRegexSentence()) {
            // TODO: student-facing message can include details + feedback should be dynamic
            // to the clause's error
            _assertTrue(clause.validate(matcher.group(matchGroupNum)), incorrectOutputMessage,
                    "Invalid Clause output at index " + matchGroupNum + " :" + clause.getInvalidMessage());
            matchGroupNum++;
        }
    }
