...
public abstract class BaseRandomTest extends BaseTest {
  final static int NUM_RUNS =
      500; // TODO: this could be generated based on ranges
  final static String MESSAGE_INCORRECT_RANDOM =
      "You do not seem to be properly generating random values.";

  @Test
  @Order(1)
  public void randomNumbersAreCorrectlyGenerated()
      throws InvalidClauseException, InvalidTestOptionException,
             InterruptedException {
    Thread.sleep(1000);
    for (int repetitionCount = 1; repetitionCount <= NUM_RUNS;
         repetitionCount++) {
      setUp();
      String output = getOutput();
      Matcher matcher = getMatches(
          output,
          processRegexForPrintlnOutput(combineRegex(getRegexSentence())));
      if (!matcher.find()) {
    ...
    // Fail case
    _fail(
        MESSAGE_INCORRECT_RANDOM,
        "While checking for randomness, an output that didn't match the test sentence was encountered.");
  }
  ...
  public void checkRandomNumberDistributions() {
    for (int i = 0; i < getRegexSentence().length; i++) {
      ...
      int mapKey = i + 1;
      // TODO: the way it's bad (out of bounds, within bounds, obviously
      // hardcoded, etc) (?)
      try {
        _assertTrue(randomVariable.validateRandom(mapKey),
                    MESSAGE_INCORRECT_RANDOM,
                    "Invalid Random Values at " + mapKey);
      } catch (Error error) {
        Logger.logMessage(String.join(" ", "Randomness Error @",
                                      randomVariable.getClass().getSimpleName(),
                                      "(", "" + mapKey, ")"));
        Logger.logMessage(
            randomVariable.getValuesForMatchGroup(mapKey).toString());
        throw error;
      }
    }
  }
